...
public class Variable {
	...
	public static void main(String[] args) {
		...
		*/
		...
		//표현범위가 큰 타입에서 작은 타입으로 형변활 할 경우 데이터가 손실될 수 있다.
		
		_double = 3.14;
		_int = (int)_double;
		System.out.println(_int); //int는 소수점을 표현할 수 없으므로 소수점이 손실된다.
		
		//표현범위가 작은 타입에서 큰 타입으로 형변환 할 경우 데이터가 손실된 염려가 없다.
		//그러므로 형변환을 생략할 수 있다.
		_int = _byte;
		_long = _int;
		_double = _int;
		...
		*/
		...
		System.out.print("문자열을 입력해주세요 : ");
		String input1 = s.nextLine();
		System.out.println("입력한 문자열 : " + input1);
		
		//숫자를 입력받고 입력받은 문자열을 출력해주세요.
		System.out.print("숫자를 입력해주세요 : ");
		int input2 = Integer.parseInt(s.nextLine());
		System.out.println("입력한 숫자열 : " + input2);
